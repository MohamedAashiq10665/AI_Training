...
def test_get_enriched_profile_combines_internal_and_external_data(client: TestClient) -> None:
    response = client.get("/customers/1/enriched-profile")

    assert response.status_code == 200
    body = response.json()
    assert body["customer"]["customer_id"] == 1
    assert body["external_activity"]["total_todos"] == 5
    assert body["weather"]["city"] == "Austin"
    assert body["warnings"] == []


def test_get_enriched_profile_returns_warning_when_no_address(client: TestClient) -> None:
    response = client.get("/customers/4/enriched-profile")

    assert response.status_code == 200
    body = response.json()
    assert body["weather"] is None
    assert "weather_unavailable: customer has no address" in body["warnings"]
