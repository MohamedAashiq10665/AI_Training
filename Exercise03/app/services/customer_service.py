...
from app.integrations.providers.interfaces import CustomerActivityProvider, WeatherProvider
...
class CustomerService:
    def __init__(
        ...
        activity_provider: CustomerActivityProvider,
    ...
    def get_enriched_customer_profile(
        self,
        customer_id: int,
        ...
        external_activity: dict | None = None
        if include_activity:
            try:
                external_activity = self.activity_provider.get_customer_activity(customer_id)
            except ExternalServiceError as error:
                warnings.append(f"activity_unavailable: {error}")

        weather: dict | None = None
        if include_weather:
            address = self._get_primary_address(customer_details["addresses"])
            if address is None:
                warnings.append("weather_unavailable: customer has no address")
            else:
                try:
                ...
                except (ExternalServiceError, IntegrationConfigError) as error:
                    warnings.append(f"weather_unavailable: {error}")
    ...
    def _get_primary_address(self, addresses: list[dict]) -> dict | None:
