...
from app.domain.errors import ConflictError, ExternalServiceError, NotFoundError
...
def test_create_customer_adds_record(service: CustomerService) -> None:
    ...
    assert service.get_customer(5)["email"] == "mia.lopez@example.com"
...
def test_assign_and_remove_customer_tag(service: CustomerService) -> None:
    tags_after_assign = service.assign_tag_to_customer(1, 3)
    assert {tag["tag_id"] for tag in tags_after_assign} == {1, 2, 3}

    tags_after_remove = service.remove_tag_from_customer(1, 3)
    assert {tag["tag_id"] for tag in tags_after_remove} == {1, 2}


def test_enriched_profile_uses_mocked_external_providers() -> None:
    activity_provider = Mock()
    weather_provider = Mock()
    activity_provider.get_customer_activity.return_value = {
        "open_todos": 1,
        "total_todos": 3,
        "recent_post_titles": ["A", "B", "C"],
    }
    weather_provider.get_current_weather.return_value = {
    ...
    service = CustomerService(MockDatabase(), activity_provider, weather_provider)

    result = service.get_enriched_customer_profile(1)

    activity_provider.get_customer_activity.assert_called_once_with(1)
    weather_provider.get_current_weather.assert_called_once_with(city="Austin", country_code="US")
    assert result["external_activity"]["total_todos"] == 3
    assert result["weather"]["condition"] == "Clouds"


def test_enriched_profile_handles_external_provider_failure() -> None:
    activity_provider = Mock()
    weather_provider = Mock()
    activity_provider.get_customer_activity.side_effect = ExternalServiceError("simulated outage")
    weather_provider.get_current_weather.return_value = {
        "provider": "openweathermap",
        "city": "Austin",
        "temperature_c": 29.0,
    ...
    assert result["external_activity"] is None
    assert "activity_unavailable: simulated outage" in result["warnings"]
